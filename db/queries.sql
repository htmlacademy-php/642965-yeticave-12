...
SELECT name_lot, price_start, image, name_cat, dt_complete
FROM lots, categories c WHERE category_id = c.id AND dt_complete > NOW()
ORDER BY dt_create DESC;

# Показать лот по его ID, название категории к которой он принадлежит и пользователя, который его создал.
SELECT l.id, name_lot, price_start, name_cat, email
...
SELECT name_lot, price, dt_add
FROM bids, lots WHERE lot_id = lots.id AND lot_id = 1
ORDER BY dt_add ASC;

# тоже самое с использованием JOIN.
SELECT name_lot, price, dt_add
FROM lots JOIN bids b ON lots.id = b.lot_id
WHERE lot_id = 1 ORDER BY dt_add ASC;

# тоже самое + имя пользователя сделавшего ставку.
SELECT name_lot, price, first_name, email, dt_add
FROM lots JOIN bids b ON lots.id = b.lot_id
JOIN users u ON b.user_id = u.id
WHERE lot_id = 1 ORDER BY dt_add ASC;

# Список ставок пользователя по его email.
SELECT first_name, email, price, name_lot
...
SELECT first_name, email, COUNT(l.id) lot_total
FROM users u JOIN lots l ON u.id = l.user_id
